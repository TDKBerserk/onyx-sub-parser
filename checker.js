...
function extractConfigsFromText(text) {
  ...
  if (!text.includes('vless://') && !text.includes('trojan://') && !text.includes('vmess://') && !text.includes('ss://')) {
    try {
      ...
      if (decoded.includes('vless://') || decoded.includes('trojan://') || decoded.includes('vmess://') || decoded.includes('ss://')) {
        text = decoded;
      }
    } catch (e) {}
  }
  ...
  linkMatches.forEach(link => list.push(link.trim().replace(/[.,;:]+$/, '')));
  return list;
}
...
async function main() {
  ...
  for (const text of rawTexts) {
    ...
    for (let line of matches) {
      ...
      const label = `onyxVPN | config_${finalConfigs.length + 1}`;
